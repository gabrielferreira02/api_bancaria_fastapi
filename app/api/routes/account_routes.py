...
async def get_balance(account_number: str, session: Session = Depends(get_session), user: User = Depends(verify_token)):
    return AccountService.get_balance(account_number, session, user)